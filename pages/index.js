...
const ERRORES_TURBOCHEF = [
  {code:"F1 – Blower Failure",nivel:"CRÍTICO",desc:"El control no recibió la señal OK del BMSC por 3 veces consecutivas. Para la cocción.",pasos:["Verificar que el circuito eléctrico tenga energía — revisar el disyuntor de pared.","Apagar el horno y desconectar el cable de alimentación por 2 minutos. Volver a conectar.","Si persiste → revisar el BMSC (Blower Motor Speed Controller CON-7013).","Verificar el motor del blower y su conexión eléctrica.","Si BMSC o motor están dañados → contratar técnico certificado TurboChef."]},
  {code:"F2 – Temperatura baja en cocción",nivel:"MODERADO",desc:"La temperatura cayó 25°F (14°C) por debajo del set point durante una cocción. NO para la cocción — solo se registra.",pasos:["Verificar que la puerta de entrada/salida del conveyor no esté bloqueada.","Verificar que los inyectores de aire (finger assemblies) estén limpios y bien instalados.","Si la temperatura es inconsistente → verificar el RTD (sonda de temperatura).","Limpiar el interior del horno — la grasa acumulada afecta la distribución de calor."]},
  {code:"F4 – Emergency Stop",nivel:"SIMPLE",desc:"Se presionó la tecla de parada de emergencia. Es un contador — no indica falla mecánica.",pasos:["Reiniciar el horno normalmente.","Este código se limpia automáticamente al reiniciar.","Si aparece frecuentemente sin que nadie presione la tecla → verificar el interruptor de emergencia."]},
  {code:"F5 – CC Over Temp (cámara sobrecalentada)",nivel:"CRÍTICO",desc:"La temperatura de la cámara de cocción excedió el límite. Para la cocción.",pasos:["Apagar el horno y dejar enfriar completamente.","Limpiar el interior — la grasa acumulada en los elementos calefactores puede encenderse y elevar la temperatura.","Verificar que los inyectores de aire no estén obstruidos — el flujo de aire es fundamental para regular la temperatura.","Verificar que la bandeja de goteo esté vacía y limpia.","Si persiste → contratar técnico certificado TurboChef."]},
  {code:"F6 – EC Over Temp (compartimiento eléctrico)",nivel:"CRÍTICO",desc:"Temperatura del compartimiento eléctrico demasiado alta. Para la cocción.",pasos:["Verificar que el horno tenga espacio libre mínimo: 254 mm arriba, 51 mm a cada lado.","Verificar que el ventilador de refrigeración posterior no esté bloqueado por objetos o paredes.","Limpiar los filtros de ventilación con compresor de aire.","NO instalar el horno cerca de freidoras o equipos que generen mucho calor.","Si persiste → contratar técnico certificado."]},
  {code:"F7 – RTD Failure",nivel:"CRÍTICO",desc:"Falla en el RTD (sonda de temperatura de cámara). Si muestra 999°F = RTD abierto o desconectado.",pasos:["Verificar la conexión del RTD a la tarjeta de control.","Un RTD desconectado muestra 999°F o 999°C en pantalla.","Verificar continuidad del cable del RTD.","Si el RTD está dañado → contratar técnico certificado para reemplazo."]},
  {code:"F8 – High Limit Tripped",nivel:"CRÍTICO",desc:"El termostato de alto límite disparó. Causado frecuentemente por grasa acumulada.",pasos:["Apagar y dejar enfriar completamente.","Limpiar el horno a fondo siguiendo el procedimiento completo — banda, inyectores, interior.","Verificar que el ventilador funcione correctamente.","El termostato de alto límite se resetea solo al enfriar — si no se resetea → contratar técnico.","PREVENTIVO: seguir el ciclo de limpieza diario rigurosamente."]},
  {code:"F9 – Belt Fault / Over Temp >650°F",nivel:"MODERADO",desc:"La banda detectó temperatura >650°F por más de 40 segundos. NO para la cocción — solo se registra.",pasos:["Verificar que la banda no tenga productos muy grasosos que generen llama o calor excesivo.","Limpiar la banda y el interior del horno.","Si el código aparece frecuentemente → revisar el tipo de producto que se está cocinando.","Verificar la cadena/banda transportadora — si está dañada puede generar fricción."]},
  {code:"Sin enfriamiento / ventilación",nivel:"MODERADO",desc:"No entra aire de refrigeración. Ventilador trasero bloqueado.",pasos:["Retirar objetos que bloqueen las rejillas de ventilación laterales y traseras.","Limpiar los filtros con compresor de aire.","Verificar distancia mínima 51 mm a cada lado y 254 mm arriba."]},
  {code:"Temperatura inconsistente",nivel:"COMÚN",desc:"El horno no alcanza o mantiene la temperatura programada.",pasos:["Verificar que los inyectores de aire (finger assemblies) estén bien instalados y limpios.","Verificar que la banda esté correctamente tensada y centrada.","Limpiar el interior del horno — grasa en los elementos calefactores reduce eficiencia.","Verificar RTD con multímetro (resistencia Clase B: 100 Ω a 0°C, Alpha = 0.00385).","Contraseña TEST MODE: 2-4-3-3 para diagnosticar componentes individualmente."]},
];
...
${esRational?"Rational SCC errores: 10=bomba SC sin función (causa más frecuente — calcificación, verificar manguera y bomba); 11/12/13=sensor CDS/llenado; 14=conductividad agua baja (<50µS); 20.1=sonda B1 cámara; 20.2=sonda B2; 20.4=sonda B4; 20.8=sonda B5 generador; 23/24=hardware apagar ya; 25=CleanJet sin agua (verificar presión y filtro); 26/27=válvula esfera CleanJet; 28=temp generador >180°C; 29=PCB caliente >85°C (limpiar filtro aire); 30=control humedad P1 falla; 31.x=sonda núcleo; 32/33=ignición gas CERRAR GAS YA; 34=bus comunicación; 36/37=sensor P1; 40/41/42=bombas care/detergente; 47/48=bomba desagüe; 100=bomba SC limpieza; 110/120=bomba SC o Care. Tabla sondas: B1=cámara B2=quenching B4=humedad B5=generador. Sondas combinadas: 20.6=B2+B4; 20.12=B4+B5.":""}
${esUnox?"Unox errores: AF01=motor sobretemperatura (capacitor 6.3µF, KCN1003A $135k); AF02=termostato seguridad 320°C (KTR1136A $405k); AF03=sonda PT100 cámara (valor correcto: 110Ω@25°C / 119Ω@50°C / 138Ω@100°C, KTR1105A $540k); AF04=comunicación RJ45 placa (KCE1095A $180k); AF08=tacómetro motor parado (imán, resistores frenado 75Ω amarillo / 27.5Ω rojo); AF13=cámara >340°C (contactores TL1/TL2 trabados); AF23=gas no ignita (presión: natural 17-25mbar / LPG 25-37mbar, ionización 4-15µA, CO2 objetivo 9.4% v/v); WF01/WF02=sonda CMB1/CMB2; WF06=placa >70°C (ventilador trasero); WF12=placa termopar >70°C; WF16=sin agua EL1 (presión 1.5-6bar, reductor 2.3bar); WF19=sin detergente; WF20/WF26=válvula EG1/EG2 lavado; WF23=presión lavado baja; WF33/AF40=tarjeta SD. Service Menu PIN: 99857. Transformer voltages: 12Vac y 21Vac (EU). Consumo lavado ciclo largo PLUS: 400g detergente, 30L agua, 103 min.":""}
${(ctx?.marca?.nombre||"").toLowerCase().includes("turbochef")?"TurboChef HHC errores: F1=blower failure (BMSC CON-7013, motor blower); F2=temperatura baja durante cocción (no para); F4=emergency stop (solo contador, no falla); F5=cámara sobrecalentada (limpiar grasa); F6=compartimiento eléctrico caliente (verificar ventilación, mín 254mm arriba 51mm lados); F7=RTD failure (999°F=desconectado, Clase B: 100Ω@0°C, Alpha 0.00385); F8=high limit tripped (limpiar grasa, causa más frecuente); F9=over temp >650°F (no para). TEST MODE contraseña: 2-4-3-3.":""}
...
function GuiaTab() {
  ...
  const errores = marca === "rational" ? ERRORES_RATIONAL : marca === "turbochef" ? ERRORES_TURBOCHEF : ERRORES_UNOX;
  return (
    <div style={{padding:14,overflowY:"auto",height:"calc(100vh - 110px)"}}>
